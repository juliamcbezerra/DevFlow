...
import { 
    ...
    MapPin, Globe, Github, Linkedin, Instagram, KeyRound, Mail, Hash, X, Moon, Sun, Monitor, CheckCircle
} from "lucide-react";

// --- Componente Auxiliar: InputGroup (Mantido) ---
...
export default function SettingsPage() {
    ...
    // Interface Settings (Mock por enquanto)
    const [theme, setTheme] = useState<'dark' | 'light'>('dark');

    // --- DADOS DO PERFIL (Mantido) ---
    ...
    const [showDeleteSuccessModal, setShowDeleteSuccessModal] = useState(false); // NOVO ESTADO AQUI
    
    const [showDeleteModal, setShowDeleteModal] = useState(false);
    const [deletePassword, setDeletePassword] = useState("");

    // Função auxiliar para fechar e limpar o modal de exclusão (Mantido)
    ...
    const toggleTag = (tag: string) => {
        ...
        setProfileData(prev => {
            const exists = prev.interestTags.includes(normalizedTag);
            return {
                ...prev,
                interestTags: exists 
                    ? prev.interestTags.filter(t => t !== normalizedTag)
                    : [...prev.interestTags, normalizedTag]
            };
        });
    };
    ...
    const handleCustomTagKeyDown = (e: React.KeyboardEvent) => {
        if (e.key === 'Enter') {
            e.preventDefault();
            const tagToAdd = normalizeTag(customTag); 
            if (tagToAdd) {
                toggleTag(tagToAdd);
                setCustomTag("");
            }
    ...
    // 1. SALVAR PERFIL (Alerts removidos, usando console.log/error)
    const handleSaveProfile = async () => {
        setLoading(true);
        try {
            const updatedUser = await userService.updateProfile(profileData);
            updateUser(updatedUser); 
            console.log("Perfil atualizado com sucesso!"); // Substitui alert()
        } catch (error) {
            console.error("Erro ao atualizar o perfil.", error); // Substitui alert()
        } finally {
            setLoading(false);
        }
    };

    // 2. RECUPERAÇÃO DE SENHA (Solicitar Código) (Alerts removidos, usando console.error)
    const handleRequestCode = async () => {
        if (!user?.email) return console.error("E-mail do usuário não disponível para redefinição."); // Substitui alert()
        ...
        try {
            ...
            console.error("Erro ao solicitar código. Verifique sua conexão ou tente novamente.", error); // Substitui alert()
        } 
        finally { setLoading(false); }
    };

    // 3. RECUPERAÇÃO DE SENHA (Confirmar Troca) (Alerts removidos, usando console.error)
    const handleConfirmPasswordChange = async () => {
        if (resetData.newPassword !== resetData.confirmPassword) return console.error("As novas senhas não coincidem."); // Substitui alert()
        ...
        try {
            ...
            console.error(error.response?.data?.message || "Erro ao trocar senha. Código inválido ou expirado.", error); // Substitui alert()
        } 
        finally { setLoading(false); }
    };

    // Ação pós-confirmação do modal de senha alterada (Alert removido)
    const handlePasswordChangedConfirmation = () => {
        setShowPasswordChangedModal(false);
        signOut();
    };


    // 4. DELETAR CONTA (Lógica atualizada para usar modal de sucesso)
    const handleDeleteAccount = async () => {
        if (!deletePassword) return console.error("A senha deve ser informada para exclusão."); // Substitui alert()

        try {
            await api.post('/users/me/delete', { password: deletePassword });
            
            closeDeleteModal(); // Fecha o modal de input de senha
            setShowDeleteSuccessModal(true); // Abre o modal de sucesso

        } catch (error: any) { 
            console.error("Senha incorreta. A exclusão foi cancelada.", error); // Substitui alert()
            setDeletePassword(""); 
            closeDeleteModal();
        }
    };

    // Ação pós-confirmação do modal de exclusão
    ...
    return (
        <AppShell>
            ...
            <div className="flex-1 min-w-0 max-w-6xl mx-auto w-full pb-20 pt-8 px-6 lg:px-12">
                    ...
                    <div className="xl:col-span-2 space-y-8">
                        ...
                        <section className="bg-zinc-900/40 border border-zinc-800 rounded-3xl p-6 md:p-8 backdrop-blur-sm">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    <InputGroup label="Nome de Exibição" value={profileData.name} onChange={(e) => setProfileData({...profileData, name: e.target.value})} />
                                    <InputGroup label="Localização" icon={MapPin} value={profileData.location} onChange={(e) => setProfileData({...profileData, location: e.target.value})} placeholder="Ex: São Paulo, Brasil" />
                                </div>
                        ...
                        <section className="bg-zinc-900/40 border border-zinc-800 rounded-3xl p-6 md:p-8 backdrop-blur-sm">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex flex-wrap gap-2 mt-2">
                                    {suggestedTags.filter(t => !profileData.interestTags.includes(normalizeTag(t))).map(tag => (
                        ...
                        <section className="bg-zinc-900/40 border border-zinc-800 rounded-3xl p-6 md:p-8 backdrop-blur-sm">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <InputGroup label="Github" icon={Github} value={profileData.socialLinks.github} onChange={(e) => setProfileData({...profileData, socialLinks: {...profileData.socialLinks, github: e.target.value}})} placeholder="Usuario" />
                                <InputGroup label="LinkedIn" icon={Linkedin} value={profileData.socialLinks.linkedin} onChange={(e) => setProfileData({...profileData, socialLinks: {...profileData.socialLinks, linkedin: e.target.value}})} placeholder="URL" />
                                <InputGroup label="Instagram" icon={Instagram} value={profileData.socialLinks.instagram} onChange={(e) => setProfileData({...profileData, socialLinks: {...profileData.socialLinks, instagram: e.target.value}})} placeholder="@usuario" />
                                <InputGroup label="Portfolio" icon={Globe} value={profileData.socialLinks.portfolio} onChange={(e) => setProfileData({...profileData, socialLinks: {...profileData.socialLinks, portfolio: e.target.value}})} placeholder="https://..." />
                            </div>
                    ...
                    <div className="space-y-8">
                        ...
                        <section className="bg-zinc-900/40 border border-zinc-800 rounded-3xl p-6 backdrop-blur-sm">
                            ...
                            {resetStep === 'idle' ? (
                                ...
                                <div className="space-y-4 animate-in fade-in zoom-in-95 duration-300">
                                    ...
                                    <InputGroup label="Código" value={resetData.code} onChange={(e) => setResetData({...resetData, code: e.target.value})} placeholder="123456" />
                                    <InputGroup label="Nova Senha" type="password" value={resetData.newPassword} onChange={(e) => setResetData({...resetData, newPassword: e.target.value})} placeholder="••••••••" />
                                    <InputGroup label="Confirmar" type="password" value={resetData.confirmPassword} onChange={(e) => setResetData({...resetData, confirmPassword: e.target.value})} placeholder="••••••••" />
                        ...
                        <section className="bg-red-500/5 border border-red-500/10 rounded-3xl p-6 backdrop-blur-sm">
                            ...
                            <p className="text-xs text-red-300/70 mb-6 leading-relaxed">
                                Excluir a conta é irreversível. Todos os seus dados serão permanentemente removidos.
                            </p>
                ...
                {/* 1. MODAL DE CONFIRMAÇÃO DO ENVIO DO CÓDIGO */}
                <ConfirmModal 
                    isOpen={showCodeSentModal}
                    onClose={() => {
                    ...
                    description={`Acabamos de enviar um código de 6 dígitos para o seu e-mail: ${user?.email}. Verifique a caixa de entrada (e o spam).`}
                ...
                {/* 2. MODAL DE CONFIRMAÇÃO DE SENHA ALTERADA */}
                <ConfirmModal 
                    isOpen={showPasswordChangedModal}
                    onClose={handlePasswordChangedConfirmation}
                    onConfirm={handlePasswordChangedConfirmation}
                    title="Senha Alterada com Sucesso!"
                    description="Sua senha foi atualizada. Para a sua segurança, você será desconectado e precisará fazer login novamente com a nova senha."
                ...
                {/* 3. MODAL DE INPUT DE SENHA PARA EXCLUSÃO (Gatilho) */}
                <ConfirmModal 
                    isOpen={showDeleteModal}
                    onClose={closeDeleteModal}
                    onConfirm={handleDeleteAccount}
                    title="Exclusão de Conta"
                    description="Esta ação é irreversível. Digite sua senha atual para confirmar a exclusão."
                    ...
                    customContent={
                        <input 
                            ...
                            className="w-full bg-zinc-950 px-4 py-3 rounded-xl border border-red-900/50 text-red-200 focus:outline-none shadow-2xl mt-4" 
                ...
                {/* 4. NOVO MODAL DE SUCESSO PÓS-EXCLUSÃO */}
                <ConfirmModal 
                    isOpen={showDeleteSuccessModal}
                    onClose={handleConfirmDelete}
                    onConfirm={handleConfirmDelete}
                    title="Conta Excluída com Sucesso"
                    description="Sua conta e todos os seus dados foram removidos permanentemente. Você será redirecionado para a página de login."
                    confirmText="Fechar e Sair"
                    isDestructive={false}
                    customContent={
                        <div className="flex justify-center my-4">
