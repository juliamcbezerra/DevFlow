...
// Removida a importação de Link, pois usaremos um button para abrir o modal
...
  isLoading?: boolean;
  error?: string;
  // Nova prop: Função para ser chamada quando o usuário clicar em "Esqueceu a senha?"
  onForgotPasswordClick: () => void; 
}

export function LoginForm({ onSubmit, isLoading = false, error, onForgotPasswordClick }: LoginFormProps) {
...
          </label>
          
          {/* Atualizado para usar um <button> que chama a função da prop */}
          <button 
            type="button" 
            onClick={onForgotPasswordClick} 
            disabled={isLoading}
            className="text-sm font-medium text-violet-500 hover:text-violet-400 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Esqueceu a senha?
          </button>
        </div>
