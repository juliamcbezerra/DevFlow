...
// Removida a importação de Link, pois usaremos um button para chamar o modal
...
  isLoading?: boolean;
  error?: string;
  // Nova prop para o clique nos termos
  onShowTermsClick: () => void; 
}

export function SignupForm({ onSubmit, isLoading = false, error, onShowTermsClick }: SignupFormProps) {
...
        {/* Termos (Usando button para abrir o modal via prop) */}
...
              Li e concordo com os 
              <button type="button" onClick={onShowTermsClick} className="text-violet-400 hover:underline px-0.5" disabled={isLoading}>Termos</button> e 
              <button type="button" onClick={onShowTermsClick} className="text-violet-400 hover:underline px-0.5" disabled={isLoading}>Privacidade</button>.
            </span>
          </label>
