...
export function Navbar() {
  ...
  const getNotifIcon = (type: string) => {
      ...
      if (type === 'INVITE') return <Code2 size={14} className="text-green-500"/>;
    ...
    <nav className="fixed top-0 left-0 right-0 h-16 border-b border-zinc-800 bg-zinc-950/80 backdrop-blur-xl z-50 px-4 md:px-6 flex items-center justify-between">
      ...
      <div className="flex items-center gap-8 z-50">
        <Link to="/feed" className="flex items-center gap-2 group">
            <motion.div 
                ...
                whileTap={{ scale: 0.95 }}
                className="bg-linear-to-tr from-violet-600 to-indigo-600 p-2 rounded-lg shadow-lg shadow-violet-500/20"
            >
      ...
      <form onSubmit={handleSearch} className="hidden md:block absolute left-1/2 -translate-x-1/2 w-96 lg:w-lg">
        ...
        <input 
            ...
            className="w-full bg-zinc-900/50 border border-zinc-800 text-zinc-200 text-sm rounded-full pl-10 pr-4 py-2 focus:outline-none focus:border-violet-500/50 focus:bg-zinc-900 focus:ring-1 focus:ring-violet-500/20 transition-all"
        />
      </form>
      ...
      <div className="flex items-center gap-3 z-50">
        ...
        <Link to="/chat" className="relative group">
            <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
                <div className="p-2 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-full transition-colors">
                ...
                {isNotificationsOpen && (
                    <>
                        ...
                        <motion.div 
                            ...
                            animate="visible"
                            exit="exit"
                            // AQUI: bg-zinc-950 sólido para não ter transparência
                            className="absolute right-0 top-full mt-3 w-80 bg-zinc-950 border border-zinc-800 rounded-xl shadow-2xl shadow-black z-50 overflow-hidden origin-top-right ring-1 ring-zinc-800"
                        >
                            ...
                            <div className="max-h-[350px] overflow-y-auto custom-scrollbar bg-zinc-950">
                                {notifications.length > 0 ? notifications.map((notif) => (
                                    <motion.div 
                                        ...
                                        animate={{ opacity: 1 }}
                                        key={notif.id} 
                                        className={`group relative p-3 border-b border-zinc-900 hover:bg-zinc-900 transition-colors flex gap-3 ${!notif.read ? 'bg-zinc-900/50' : ''}`}
                                    >
                                        ...
                                        <div className="absolute right-2 top-2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-zinc-950 border border-zinc-800 rounded-lg p-0.5">
                                            {!notif.read && (
                                                <button onClick={(e) => handleMarkAsRead(e, notif.id)} className="p-1 text-zinc-400 hover:text-green-400 hover:bg-green-400/10 rounded">
                                                    <Check size={12}/>
                                                </button>
                                            )}
                                            <button onClick={(e) => handleDeleteNotification(e, notif.id, notif.read)} className="p-1 text-zinc-400 hover:text-red-400 hover:bg-red-400/10 rounded">
                                                <X size={12}/>
                                            </button>
                                        </div>
                ...
                {isDropdownOpen && (
                    <>
                        ...
                        <motion.div 
                            ...
                            animate="visible"
                            exit="exit"
                            className="absolute right-0 top-full mt-3 w-56 bg-zinc-950 border border-zinc-800 rounded-xl shadow-2xl z-50 overflow-hidden origin-top-right ring-1 ring-zinc-800"
                        >
                            <div className="p-3 border-b border-zinc-800 bg-zinc-900">
                                <p className="text-sm font-bold text-white truncate">{user?.name}</p>
                                <p className="text-xs text-zinc-500 truncate">@{user?.username}</p>
                            </div>
                            <div className="p-1.5 space-y-0.5 bg-zinc-950">
                                <Link to={`/profile/${user?.username}`} className="flex items-center gap-2 px-3 py-2 text-sm text-zinc-300 hover:bg-zinc-900 rounded-lg transition-colors"><User size={16}/> Meu Perfil</Link>
                                <Link to="/settings" className="flex items-center gap-2 px-3 py-2 text-sm text-zinc-300 hover:bg-zinc-900 rounded-lg transition-colors"><Settings size={16}/> Configurações</Link>
                            </div>
